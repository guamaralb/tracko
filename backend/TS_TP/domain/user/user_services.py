...
def user_service_create(*, uow: UnitOfWork, data: UserCreateSchema):
    pwd_hash = data.password # CORRIGIR
    
    new_user = UserModel(
        email=data.email, pwd_hash=pwd_hash, name=data.name
    )

    return uow.users.add(new_user)


def user_service_read_one(*, uow: UnitOfWork, user_id: UUID):
